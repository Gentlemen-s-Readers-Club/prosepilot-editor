        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
          {books.map((book) => (
            <div
              ...
              className={`group transition-all duration-300 ${book.status !== 'writing' ? 'cursor-pointer' : 'cursor-default'}`}
            >
        ...
        <div className="space-y-4">
          {books.map((book) => (
            <div
              key={book.id}
              onClick={() => book.status !== 'writing' && navigate(`/app/book/${book.id}`)}
              className={`group bg-white border border-gray-200 rounded-lg p-4 flex gap-4 transition-all hover:shadow-md ${
